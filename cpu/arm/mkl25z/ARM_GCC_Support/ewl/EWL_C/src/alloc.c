...
#if defined(__COLDFIRE__) && __COLDFIRE__
...
#elif defined(__POWERPC__) && __POWERPC__
#define FAR
#define _MW_OLD_BRK_ROUTINE 	1
#define HEAP_GROWS 		1
...
static struct m_buff** tail = &head;	/* tail of the buffer list */
	...
	#if defined(__arm) && __arm
		extern FAR char_t	_EWL_HEAP_START[]; 		/* linker generated symbol */
		extern FAR char_t	__SP_INIT[];			/* linker generated symbol */
		static char_t* __break = _EWL_HEAP_START;
	#else
...
int_t brk ( void* addr )
{
	...
	MISRA_EXCEPTION_RULE_13_7()
	    ...
	    if ((compare >= __mem_limit) ||
			((compare >  (stp - (int32_t)__stack_safety)) && (__SP_INIT > __mem_limit)))
	#else
    if ((compare >= __mem_limit) ||
    	...
    	** the dynamic configuration means that the stack and heap are shared the same memory space.
		...
		char_t* stp;
     	...
     	if (!((uint32_t)readSR() & ((uint32_t)0x1u << 18))) {
            ...
            mem_limit_end = stp + __stack_safety;
        } else {
