...
/* $Id: e_hypot.c,v 1.3 2010/05/12 07:07:15 anczane1 Exp $ */
/*
 * ====================================================
...
#endif
{
	...
	ha = __UHI(x)&0x7fffffffUL;	/* high word of  x */
	hb = __UHI(y)&0x7fffffffUL;	/* high word of  y */
	if(hb > ha) {
		uint32_t j;
	...
	if(ha > 0x5f300000UL) {	/* a>2**500 */
	   if(ha >= 0x7ff00000UL) {	/* Inf or NaN */
	       w = a+b;			/* for sNaN */
	       if(((ha&0xfffffUL)|__ULO(a))==0UL) {w = a;}
	       if(((hb^0x7ff00000UL)|__ULO(b))==0UL) {w = b;}
			MISRA_EXCEPTION_RULE_14_7()
	       return w;
	   }
