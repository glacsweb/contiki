...
{
    if ( __write_file(h, (unsigned char *)buf, (unsigned long *)&len) == __no_io_error) {
        return (len);
    }
    else {
        return -1;
...
{
    if ( __read_file(h, (unsigned char *)buf, (unsigned long *)&len) == __no_io_error ) {
        return (len);
    }
    else
        return -1;
 ...
 *
 */
__declspec(weak) void __init_semihost(void)
{
