...
/*
 *	HandleTerminate - wrapper function for the terminate handler
 *
 *	When the terminate handler needs calling, rethrow and catch the exception so that it may be considered "caught"
 *	when entering the user terminate function. 
 *
 */
static void HandleTerminate(ThrowContext* context)
{
	try 
	{
		__throw(context->throwtype, context->location, context->dtor);
	}
	catch( ... )
	{
		terminate();
...
static char *FindExceptionHandler(ThrowContext *context, ExceptionInfo *info, long *result_offset)
{
	...
	for (action = CurrentAction(&iter);; action = NextAction(&iter)) {
		switch (action) {
			...
			case EXACTION_TERMINATE:
			default:
				HandleTerminate(context);	/*	cannot find matching catch block	*/
		}
		break;
	}
